...
engine = create_engine(
    'sqlite:///restaurantmenuapp.db', connect_args={'check_same_thread': False}
    )
...
def fbconnect():
    ...
    '''
        ...
        actual token value
        and replace the remaining quotes with nothing so that it can be used d\
        irectly in the graph
        api calls
    '''
    token = result.split(',')[0].split(':')[1].replace('"', '')

    url = 'https://graph.facebook.com/v2.8/me?access_token=%s&fields=name,id,em\
    ail' % token
    h = httplib2.Http()
    result = h.request(url, 'GET')[1]
    ...
    output += ' " style = "width: 300px; height: 300px;border-radius: 150px;-w\
    ebkit-border-radius: 150px;-moz-border-radius: 150px;"> '

    flash("Now logged in as %s" % login_session['username'])
    return output

# Disconnect FB login session.
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        response = make_response(json.dumps('Failed to revoke token for given u\
        ser.', 400))
        response.headers['Content-Type'] = 'application/json'
        return response
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You are not authorized \
        to delete menu items to this restaurant. Please create your own resta\
        urant in order to delete\
